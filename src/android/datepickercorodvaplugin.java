...
/**
 * Cordova main class , which could receive call from the javascript file. this
 * class is responsible for call to the other java file and source code.
 *
 * this @class datepicker would be calling android platfrom datepicker and will
 * result in jsonobject with date selected by the user.
 * 
 * @result { "date":31/07/1994 }
 */
public class datepickercordovaplugin extends CordovaPlugin {
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("showDateMethod")) {
            if (args.length() != 0) {
                // when the default , date is provided by the first json object of the array,
                // and array contain the json object.
                ...
                // for now result is no_result for the plugin and call back is true, hence
                // resukt would be below,
                // sent after use input
                PluginResult result = new PluginResult(PluginResult.Status.NO_RESULT);
                result.setKeepCallback(true);
            } else {
                // when the default , date is not provided by the first json object of the
                // array, and array is empty.
    ...
    protected void DatePickerShow(int id, int year, int month, int day) {
        if (id == 999) {
            ...
            DatePickerDialog datePicker = new DatePickerDialog(this.cordova.getActivity(),
    ...
    protected void setDate(String date) {
        ...
        // would be sent to the user in cordova, where JS function give the result to
        // the user.
